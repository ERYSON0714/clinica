            ...
            if (item.id && item.lastModified) {
              await connection.query(
                `INSERT INTO client_storage (storage_key, storage_value, last_modified)
                 ...
                 storage_value = CASE WHEN VALUES(last_modified) > last_modified THEN VALUES(storage_value) ELSE storage_value END,
...
router.get('/download', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT storage_key, storage_value, last_modified FROM client_storage');
    const data = {};
    rows.forEach(row => {
      try {
        ...
        if (key.includes('_')) {
          // Chaves compostas (ex: db_pacientes_v3_123)
          const parts = key.split('_');
          const id = parts.pop();
          const baseKey = parts.join('_');
          if (!data[baseKey]) data[baseKey] = [];
          data[baseKey].push({ ...value, id, lastModified: row.last_modified });
        } else {
          data[key] = { ...value, lastModified: row.last_modified };
        }
      } catch (e) {
